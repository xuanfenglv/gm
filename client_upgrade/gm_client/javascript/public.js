...
$(function() {
	...
	$('.side-nav').on("click", ".side-menu dt", function() {
		let li = $(this).parent().parent();
		if(li.hasClass('open')) {
			li.removeClass('open');
		} else {
			li.addClass('open');
			li.parents('.side-menu').find('.InitialPage').removeClass('active');
//			li.siblings().removeClass('open');
		}
	});
	$('.side-nav').on("click", ".side-menu span", function() {
		let href = $(this).attr('href');
		let content = $("#content");
		let module = moduleMap.get(href);
		if(!module) {
			content.load(href,()=>{
				module = content.html();
				moduleMap.set(href,module);
			});
		} else {
			content.html(module);
		}
