...
import com.longma.mopet.gm.interfaces.IndexedEnum;
import com.longma.mopet.gm.util.PoiUtils;
import com.longma.mopet.gm.util.PoiUtils2;
...
public class TemplateObjectAssembler {
	...
	public static final Map<Class<?>, Field[]> classFields = new HashMap<Class<?>, Field[]>(30, 0.8f);
	public static final Map<Class<?>, Method[]> classMethods = new HashMap<Class<?>, Method[]>(30, 0.8f);

	public void doAssemble(Object obj, Row row, Class<?> clazz) throws Exception {
		this.doAssemble(obj, row, clazz, 0);
	}

	public void doAssemble(Object obj, Row row, Class<?> clazz, int startOffset) throws Exception {
		Class<?> superClazz = clazz.getSuperclass();
		// 如果父类是一个绑定类，那么先处理
		if (superClazz != null && superClazz.isAnnotationPresent(ExcelRowBinding.class)) {
			doAssemble(obj, row, superClazz, startOffset);
		}
		Field[] fields = null;
		...
		for (int i = 0; i < fields.length; i++) {
			if ((fields[i].getModifiers() & Modifier.STATIC) != 0) {
				...
				Object fValue = getFieldValue(fields[i], row, fieldType, binding.offset() + startOffset);
				invokeSetMethod(clazz, fields[i], obj, fValue);
				// 处理本身是绑定的字段
			} else if (fields[i].isAnnotationPresent(ExcelRowBinding.class)) {
				ExcelRowBinding rowBinding = fields[i].getAnnotation(ExcelRowBinding.class);
				Class<?> fieldType = fields[i].getType();
				Object subObject = fieldType.newInstance();
				invokeSetMethod(clazz, fields[i], obj, subObject);
				doAssemble(subObject, row, fieldType, startOffset + rowBinding.startOffset());
			} else if (fields[i].isAnnotationPresent(ExcelCollectionMapping.class)) {
				// FIXME fay 2013-3-14 集合暂时不支持偏移
	 ...
	 */
	public static boolean invokeSetMethod(Class<?> clazz, Field field, Object obj, Object value) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException, SecurityException, NoSuchMethodException {
	...
	private void insertSetOrList(Collection<Object> col, ExcelCollectionMapping ecm, Row row) throws Exception {
		...
		for (String str : cns) {
			...
			if (element_cl.isAnnotationPresent(ExcelRowBinding.class))
				...
				if (strs.length > 1)
					throw new ConfigException(String.format("cell's number greater than 1 on row(%d)", row.getRowNum()));
				o = this.getFieldValue(null, row, element_cl, new Integer(strs[0]));
			}
			col.add(o);
		}
	...
	private Object getElementObject(Class<?> clazz, String[] strs, Row row) {
		...
		try {
			...
			for (int j = 0; j < strs.length; j++) {
				...
				fValue = getFieldValue(curField, row, curField.getType(), number);
				invokeSetMethod(curField.getDeclaringClass(), curField, member_obj, fValue);
			}
		} catch (Exception e) {
			throw new ConfigException("SheetName:" + row.getSheet().getSheetName() + ",rowNum:" + row.getRowNum(), e);
		}
		return member_obj;
	}
	 ...
	 * @throws Exception
	 */
	private Object getFieldValue(Field field, Row row, Class<?> fieldType, int offset) throws Exception {
		Cell cell = row.getCell(offset);
		if (fieldType == int.class || fieldType == Integer.class) {
			return PoiUtils2.getIntValue(cell);
		} else if (fieldType == long.class || fieldType == Long.class) {
			return (long) PoiUtils2.getDoubleValue(cell);
		} else if (fieldType == short.class || fieldType == Short.class) {
			return PoiUtils2.getShortValue(cell);
		} else if (fieldType == byte.class || fieldType == Byte.class) {
			return PoiUtils2.getByteValue(cell);
		} else if (fieldType == double.class || fieldType == Double.class) {
			return PoiUtils2.getDoubleValue(cell);
		} else if (fieldType == float.class || fieldType == Float.class) {
			return PoiUtils2.getFloatValue(cell);
		} else if (fieldType == Date.class) {
			return PoiUtils2.getDateValue(cell, null);
		} else if (fieldType == Calendar.class) {
			return PoiUtils2.getCalendarValue(cell);
		} else if (fieldType == String.class) {
			String str = PoiUtils.getStringValue(cell);
			return str;
		} else if (fieldType == Boolean.class || fieldType == boolean.class) {
			int v = PoiUtils2.getIntValue(cell);
			if (v == 0)
				return Boolean.FALSE;
			if (v == 1)
				return Boolean.TRUE;
			throw new ConfigException("boolean type value error :" + v);
		} else if (fieldType.isEnum() && IndexedEnum.class.isAssignableFrom(fieldType)) {
			int v = PoiUtils2.getIntValue(cell);
	...
	private void convertValueToType(String[] values, Object result, Class<?> fieldType) {
		...
		if (fieldType == int.class || fieldType == Integer.class) {
			...
			for (int i = 0; i < values.length; i++) {
				int v = (int) Double.parseDouble(values[i]);
				boolean isSet = false;
				IndexedEnum[] enums = (IndexedEnum[]) fieldType.getEnumConstants();
